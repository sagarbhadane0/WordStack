...
public class MainActivity extends AppCompatActivity {
    ...
    private Stack<Character> stackWord1=new Stack<Character>();
    private Stack<Character> stackWord2=new Stack<Character>();
    private String Mix="";
    @Override
    ...
    public String getWord()
    {
        ...
        int min=0;
        int max=words.size();
        str = words.get(random.nextInt((max - min) + 1) - min);
        return str;
    ...
    public boolean onStartGame(View view) {
        ...
        for(i=0;i<word1.length()+word2.length();i++)
        {
            ...
            {
                if(!stackWord1.isEmpty()) {
                    Mix = Mix + stackWord1.pop().toString();
            ...
            else
            {
                if(!stackWord2.isEmpty()) {
                    Mix=Mix+stackWord2.pop().toString();
                }
                else
                    i--;
            }
        ...
        i=Mix.length()-1;
        while(0<=i){
            stackedLayout.push(new LetterTile(this , Mix.charAt(i)));
            i--;
         }
    ...
    public boolean onUndo(View view) {
        ...
        if(!placedTiles.isEmpty()) {
            ...
            if (((View)placedTiles.peek().getParent()).getId() == R.id.word1){
                word1 = new StringBuilder(word1).deleteCharAt(word1.length()-1).toString();
                placedTiles.pop().moveToViewGroup(stackedLayout);
            }
            else {
                word2 = new StringBuilder(word2).deleteCharAt(word2.length()-1).toString();
                placedTiles.pop().moveToViewGroup(stackedLayout);
            }
        }
        /**
    ...
    protected void checkWin() {
        ...
        TextView messageBox = (TextView) findViewById(R.id.message_box);
        if(word1.equals(word1) && word2.equals(word2))
            messageBox.setText("You win! " + word1 + " " + word2);
        else if(words.contains(word1) && words.contains(word2)){
            messageBox.setText("You found alternative words! " + word1 + " " + word2);
        }
        else{
